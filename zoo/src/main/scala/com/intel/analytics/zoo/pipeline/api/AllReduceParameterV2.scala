...
object AllReduceParameterV2 {
  ...
  def newParameter[T: ClassTag](
                                 partitionNum: Int,
                                 size: Int,
                                 offset: Int = 1)(implicit ev: TensorNumeric[T]): AllReduceParameterV2[T] = {
    new AllReduceParameterV2(nextId.getAndIncrement(), partitionNum, size, offset)
  }
}
...
class AllReduceParameterV2[T: ClassTag](
  ...
  def aggregateGradientPartition(avgNumbers: Int): Unit = {
      ...
      new Callable[Int] {
        override def call(): Int = {
          try {
            ...
            println(s"sync time is ${(endTime - startTime)/1e6}ms")
    ...
    println(s"all sync time is ${(time2 - time1)/1e6}ms")
