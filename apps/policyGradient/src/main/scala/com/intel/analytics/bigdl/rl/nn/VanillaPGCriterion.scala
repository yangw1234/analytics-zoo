  ...
  override def updateOutput(input: Tensor[T], target: Tensor[T]): T = {
    ...
    if (input.dim() == 1) {
      require(input.dim() == target.dim(),
        "VanillaPGCriterion: " + ErrorInfo.constrainInputDimSameAsTarget +
      ...
      require(target.dim() == 1,
        "VanillaPGCriterion: illegal target! Target should be 1D tensor after squeeze," +
...
object VanillaPGCriterion {
  def apply[@specialized(Float, Double) T: ClassTag](
    ...
    new VanillaPGCriterion[T](weights, sizeAverage)
  }
}
